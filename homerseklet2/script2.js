...
function update(objArray) { //Frissítjük a táblázatot
    ...
    objArray.forEach(obj => {
        headerItems = Array.from(header1.childNodes).slice(1, 45)
        headerItems.forEach(item =>{
            if (item.childNodes[0].childNodes[0].data == obj.name){
                changeValues(item, obj)
            }
        })
        headerItems = Array.from(header2.childNodes).slice(0, 45)
        headerItems.forEach(item =>{
            if (item.childNodes[0].childNodes[0].data == obj.name){
                changeValues(item, obj)
            }
...
function createDivs(objArray) {
    ...
    objArray1 = objArray.splice(0, 44)
    objArray1.forEach(obj => {
        addElement(header1, obj)
    })
    leftover = 44 - objArray1.length
    ...
    objArray2 = objArray.splice(0, 45)
    objArray2.forEach(obj => {
        addElement(header2, obj)
    })
    leftover = 45 - objArray2.length
    for (i = 0; i < leftover; i++) {
